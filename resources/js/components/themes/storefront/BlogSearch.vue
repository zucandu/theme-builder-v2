                ...
                <div class="col-lg-8">
                    ...
                    <div class="row mt-5 justify-content-center" v-if="!noItem">
                        <div class="col-12 text-end">
                            <router-link :to="{ path: `/blog/search`, query: { ...global_all_url_params(['page']), page: global_url_param(link.url, 'page') }}" v-for="(link, index) in paginationLinks" :key="index" :class="`btn btn-outline-dark mx-1${(!link.url ? ' disabled' : '')}${(link.active === true ? ' btn-primary text-white' : '')}`"><span v-html="link.label"></span></router-link>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    created() {
        ...
        this.queryPostListing(this.global_all_url_params())
    },
    methods: {
        queryPostListing(params) {
            this.$store.dispatch('tool_post_listing_by_keyword', { objParams: params }).catch(error => {
    ...
    computed: {
        ...mapGetters(['setting_translation', 'setting_trans_obj', 'global_url_param', 'global_all_url_params', 'account_customer_access_token']),
        ...mapState({
