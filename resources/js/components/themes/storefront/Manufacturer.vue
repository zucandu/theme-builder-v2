<template>
    <div class="zuc-listing-container row g-3 justify-content-center">
        ...
        <div :class="`zuc-listing-products ${!noProduct ? `col-lg-9 offset-lg-1` : `col-lg-12`}`">
                        ...
                        <div class="product-widget__inner mb-3">
                            ...
                            <template v-else>
                                <div class="inner__cart d-flex w-100 bottom-0 mt-3">
                                    <button @click.prevent="showModal = true, picked = { id: item.id, name: setting_translation(item, 'name', $i18n.locale) }" class="btn btn-info btn-sm text-white w-100" type="button">
            ...
            <listing-object-info :object="manufacturerTranslation"></listing-object-info>
...
export default {
    ...
    methods: {
        queryProductListing(slug, params) {
            ...
            this.$store.dispatch('catalog_listing_by_manufacturer', {slug: slug, objParams: params}).catch(error => {
        ...
        addToCart(item, qty = 1) {
            ...
            this.$store.dispatch('cart_add_product', { id: item.id, cart_quantity: qty }).then(() => {
                this.$store.commit('SETTING_SET_ALERT', {
                    'color': 'success', 
