                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 mb-3">
                        <template v-if="regions.length > 0">
                            ...
                            <select name="region" id="region" class="form-select" v-model="shippingCalculator.zone_code">
                                <option v-for="(region, index) in regions" :value="region.code" :key="index">{{ region.name }}</option>
                            </select>
                        </template>
                        <template v-else>
                            <label for="region" class="form-label">{{ $t('Region') }}</label>
                            <input v-model="shippingCalculator.zone_name" class="form-control" id="region" :placeholder="$t('Enter your region')" required>
                        </template>
                    </div>
                    <div class="col-lg-4 mb-3">
                        <label for="postcode" class="form-label">{{ $t('Postcode') }}</label>
                        <input v-model="shippingCalculator.postcode" class="form-control" id="postcode" :placeholder="$t('Enter your zip code')" required>
                    </div>
...
export default {
    ...
    created() {
        ...
        if(this.countries.length === 0) {
            this.$store.dispatch('country_list').then(() => this.shippingCalculator.country_code = this.countries.find(country => country.id > 0).iso_code_2)
        } else {
            this.shippingCalculator.country_code = this.countries.find(country => country.id > 0).iso_code_2
        }

        // Get shipping rates if the postcode is set
        if(this.shippingCalculator.postcode) {
        ...
        submitEstimateShippingCosts() {
            ...
            this.$store.dispatch('cart_estimate_shipping_costs', {
                customer: {
                    addresses: [
                        {
                            country_code: this.shippingCalculator.country_code,
                            zone_code: this.shippingCalculator.zone_code,
                            zone_name: this.shippingCalculator.zone_name,
                            postcode: this.shippingCalculator.postcode
                        }
                    ]
                },
    ...
    computed: {
        ...
        ...mapState({
            ...
            shippingCalculator: state => state.cart.shippingCalculator,
            estimateShippingCost: state => state.cart.estimateShippingCost,
            profile: state => state.customer.profile,
        }),
        regions: function() {
            const regions = this.country_get_zones_by_country_code(this.shippingCalculator.country_code)
            if(regions.length > 0 && regions.find(r => r.code === this.shippingCalculator.zone_code) === undefined) {

                this.shippingCalculator.zone_code = regions.find(r => r.id > 0).code

                // Reset zone name
                this.shippingCalculator.zone_name = undefined
            }
            return regions
        },
    },
