<template>
    <section v-if="loaded" class="section-crosssells row mt-5">
        <template v-if="loadedCrosssell">
            <template v-if="crosssells.length > 0">
                <img @load="initTinySliderCrossSell" src="/storage/pixel.gif" alt="js" class="d-none">
                <div class="col-12 h3 fw-light pb-5 mb-0"><span class="fw-bold text-dark">{{ $t("You might also be") }}</span> {{ $t('interested in') }}</div>
                <div :class="`col-12 position-relative ${loadingTinySliderCrossSell ? 'd-none' : ''}`">
                    <div class="z-tiny-slider-crosssell-section">
                        <product-widget :products="crosssells" @pickItemRestock="pickItemRestock"></product-widget>
                    </div>
                </div>
                <template v-if="loadingTinySliderCrossSell">
                    <product-widget-loading></product-widget-loading>
                </template>
            </template>
        </template>
    </section>
    <section v-else class="section-crosssells row mt-5">
...
export default {
    data: () => ({
        loadedCrosssell: false,
        crosssells: [],
        loadingTinySliderCrossSell: true,
        picked: {
            id: undefined,
            name: undefined
        },
    ...
    methods: {
        ...
        initTinySliderCrossSell() {
            tns({
                container: '.z-tiny-slider-crosssell-section',
                items: 2,
                controlsText: [
            ...
            this.loadingTinySliderCrossSell = false
        },
        pickItemRestock(v) {
            this.picked = v
            this.showModal = true
        ...
        loaded(v) {
            ...
            if(ids.length > 0) {
                axios.get(`/api/v1/storefront/product/ids/${ids.join('_')}`)
                        .then(res => this.crosssells = res.data.products)
                        .finally(() => this.loadedCrosssell = true)
            }
        }
    }
}
